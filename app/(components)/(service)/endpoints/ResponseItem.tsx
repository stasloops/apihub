		...
		<div className={styles.endpoints__response}>
			<div className={styles.endpoints__response_item}>
				<div onClick={() => setisOpenMessage(!isOpenMessage)} className={styles.endpoints__response_message}>
					<span className={styles.endpoints__response_message_code}>{response.code}</span>
					<span className={styles.endpoints__response_message_text}>{response.message}</span>
					<span style={{ transform: isOpenMessage ? 'rotate(90deg) translate(5px, 3px)' : '' }} className={styles.endpoints__response_message_arr}>
						{svg.ar}
					</span>
				</div>
