    ...
    return (
        <div className={styles.endpoints__menu_create_form}>
            <div className={styles.endpoints__menu_create_form_top}>
                    ...
                    </div>
                    {
                        typePopupIsActive ?
                            <div className={styles.endpoints__type_items}>
                                {
                                    types.map((item, id) => (
                                        <div style={{ backgroundColor: typeValue === item.name ? '#1faee9' : '' }} onClick={() => changeType(item.name)} key={id} className={styles.endpoints__type_item}>
                                            {item.name}
                                        </div>
                                    ))
                                }
                ...
                <span onClick={addParam} className={styles.endpoints__menu_create_button}>+</span>
                <span onClick={close} className={styles.endpoints__menu_create_close}>+</span>
            </div>
